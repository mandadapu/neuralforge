...
func New(cfg config.Config) (*App, error) {
	...
	if a.ghApp != nil {
		mux.Handle("/webhooks/github", a.ghApp.WebhookMiddleware(
			http.HandlerFunc(a.ghAppWebhookHandler),
		))
	} else {
...
// ghAppWebhookHandler handles webhook requests authenticated via GitHub App middleware.
func (a *App) ghAppWebhookHandler(w http.ResponseWriter, r *http.Request) {
	body, _ := io.ReadAll(r.Body)
	eventType := r.Header.Get("X-GitHub-Event")
	go a.handleEvent(eventType, body)
	w.WriteHeader(http.StatusOK)
	if _, err := w.Write([]byte(`{"ok":true}`)); err != nil {
		slog.Error("failed to write ghapp webhook response", "error", err)
	}
}
